# Vibe CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages Vibe sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `vibe daemon start`

Control Flow:
1. `src/index.ts` receives `daemon start` command
2. Spawns detached process via `spawnVibeCLI(['daemon', 'start-sync'], { detached: true })`
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-vibe-session`, `stop-session`, `requestShutdown` handlers
   - Heartbeat loop: every 60s (or VIBE_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
The daemon detects when `npm upgrade vibe-cli` occurs:
...
3. If mismatch detected:
   - Spawns new daemon via `spawnVibeCLI(['daemon', 'start'])`
...
1. Backend forwards RPC `spawn-vibe-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached Vibe process with `--vibe-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New Vibe process:
   - Creates session with backend, receives `vibeSessionId`
   - Calls `notifyDaemonSessionStarted()` to POST to daemon's `/session-started`
5. Daemon updates tracking with `vibeSessionId`, resolves awaiter
6. RPC returns session info to mobile app

### Terminal-Spawned Sessions

User runs `vibe claude` directly:
1. CLI auto-starts daemon if configured
2. Vibe process calls `notifyDaemonSessionStarted()` 
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'vibe directly...'`
4. Session tracked for health monitoring

### Session Termination

Via RPC `stop-session` or health check:
1. `stopSession()` finds session by `vibeSessionId`
...
`vibe doctor` uses `ps aux | grep` to find all Vibe processes:
- Production: matches `vibe.mjs`, `vibe-cli`, `dist/index.mjs`
...
`vibe doctor clean`:
1. `findRunawayVibeProcesses()` filters for likely orphans
2. `killRunawayVibeProcesses()`:
   - Sends SIGTERM
   - Waits 1 second
...
- Server to Daemon: rpc-request (spawn-vibe-session, stop-session, requestShutdown)
...
```json
{
  ...
  "metadata": "base64(encrypted({
    ...
    'platform': 'darwin',
    'vibeCliVersion': '1.0.0',
    'homeDir': '/Users/john',
    'vibeHomeDir': '/Users/john/.vibe'
  }))",
...
socket.emit('machine-update-metadata', {
  ...
  "metadata": "base64(encrypted({
    'host': 'MacBook-Pro.local',
    'platform': 'darwin',
    'vibeCliVersion': '1.0.1',  // version updated
    'homeDir': '/Users/john',
    'vibeHomeDir': '/Users/john/.vibe'
  }))",
  "expectedVersion": 1
